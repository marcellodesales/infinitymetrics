...
abstract class BaseEvent extends BaseObject  implements Persistent {
	...
	public function setChannelId($v)
	{
		...
		if ($this->channel_id !== $v) {
			$this->channel_id = $v;
			$this->modifiedColumns[] = EventPeer::CHANNEL_ID;
		}
	...
	public function setJnUsername($v)
	{
		...
		if ($this->jn_username !== $v) {
			$this->jn_username = $v;
			$this->modifiedColumns[] = EventPeer::JN_USERNAME;
		}

		if ($this->aUser !== null && $this->aUser->getJnUsername() !== $v) {
			$this->aUser = null;
		}
	...
	public function hasOnlyDefaultValues()
	{
			...
			if (array_diff($this->modifiedColumns, array(EventPeer::DATE))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->jn_username = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
	...
	public function ensureConsistency()
	{
		...
		if ($this->aUser !== null && $this->jn_username !== $this->aUser->getJnUsername()) {
			$this->aUser = null;
		}
	} // ensureConsistency
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EventPeer::CHANNEL_ID)) $criteria->add(EventPeer::CHANNEL_ID, $this->channel_id);
		if ($this->isColumnModified(EventPeer::EVENT_ID)) $criteria->add(EventPeer::EVENT_ID, $this->event_id);
		if ($this->isColumnModified(EventPeer::JN_USERNAME)) $criteria->add(EventPeer::JN_USERNAME, $this->jn_username);
	...
	{
		if ($v === null) {
			$this->setJnUsername(NULL);
		} else {
			$this->setJnUsername($v->getJnUsername());
		}
	...
	{
		if ($this->aUser === null && (($this->jn_username !== "" && $this->jn_username !== null))) {
			$this->aUser = UserPeer::retrieveByPK($this->jn_username, $con);
			/* The following can be used additionally to
			   guarantee the related object contains a reference
			   to this object.  This level of coupling may, however, be
			   undesirable since it could result in an only partially populated collection
