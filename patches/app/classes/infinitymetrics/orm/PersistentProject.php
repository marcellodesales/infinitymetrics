...
class PersistentProject extends PersistentBaseProject {
    ...
    public function getDistinctEventAuthors() {
        /*
         *  THIS IS SHOULD WORK, BUT IT ISN'T...
        $criteria = new Criteria();
        $criteria->setDistinct();
        $criteria->addSelectColumn(PersistentEventPeer::JN_USERNAME);
        $criteria->add(PersistentEventPeer::PROJECT_JN_NAME, $this->project_jn_name);

        return PersistentEventPeer::doSelect($criteria);
        */

        $con = Propel::getConnection(PersistentEventPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
        $sql = "SELECT DISTINCT  event.JN_USERNAME FROM `event` WHERE event.PROJECT_JN_NAME='".$this->getProjectJnName()."'";
